...
Then, we merged the information from the ABRicate results and identified those IS disrupting AMR genes using the R script **DBSCRIPT1** for the Enterobacterales order, and **DBSCRIPT2** for the rest of bacteria. The statistical analyses and representation of the whole BV-BRC database analyses plots are summarized in **DBSCRIPT2**.
