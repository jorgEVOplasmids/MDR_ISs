...
setwd("/media/jorge/acce4b86-3c48-4146-9cd1-ada046e8124b/database_IS1.2/abricate_summary_results")
...
# Filter by Enterobacteriaceae family, in the case of only analyzing a specific family

enterobacteria_metadata <- complete_genomes_metadata %>% filter(Family == "Enterobacteriaceae")

rm(complete_genomes_metadata) # Remove full table to save mem
...
for (row in 2:length(rownames(target_genes))) {
  if (target_genes[row-1,]$GENE == target_genes[row,]$GENE) { # Compare row with previous row, the extremes of the genes are reflected in 2 columns with matching gene names, and filter out genes which confer resistance through SNPs
    ...
    if (breakpoint_range < 60000 & breakpoint_range > 700) { # Filter by coords matching the criteria
...
####################### Save output #######################
#write.xlsx(KOs_by_ISs, "KOs_w_targets_by_ISs.xlsx", sheetName = "KOs_by_ISs")

# Load to avoid running the loop each execution

KOs_by_ISs <- read.xlsx("KOs_w_targets_by_ISs.xlsx", sheetIndex = 1)
...
# Now, analyze those mediated by IS elements (keep out potential transposons)
# First, merge metadata

KOs_by_ISs <- merge(KOs_by_ISs, enterobacteria_metadata, by ="Genome.ID")
...
#write.xlsx(KOs_by_ISs, "KOs_by_ISs_w_targets_w_Tns.xlsx")
